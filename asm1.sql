...
-- 4/ (nang cao) 
SET @salary_3rd = 0;
SELECT DISTINCT salary into @salary_3rd
 FROM employees
 ORDER BY salary
 DESC LIMIT 2,1;
SELECT employee_id, first_name, last_name, job_id, salary
 FROM employees
 WHERE salary = @salary_3rd;
...
COUNT(m.manager_id) AS number_of_reporters
FROM employees e
JOIN employees m 
...
-- 9/
SELECT ROUND(MIN(salary),0) AS min_sal,
ROUND(MAX(salary),0) AS max_sal,
ROUND(AVG(salary),0) AS avg_sal
FROM employees;

-- 10/
SELECT CONCAT(first_name, ' ', last_name) AS employee, salary, 
CASE WHEN salary >=2000 AND salary <5000 THEN 'low'
